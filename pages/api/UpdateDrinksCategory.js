...
export default async function UpdateDrinkCategory(req, res) {
  if (req.method == "POST") {
    try {
      DbConnection();
    ...
    return res.status(401).json({ message: "Please login with admin credentails" });
      ...
      if (IdMatching.DrinkCategoryName == DrinkCategoryName) {
        return res.status(400).json({
          message: "Please Enter New Drink Category Name Not Same Drink Name",
          status: "400",
        });
      }
      ...
      for (let i = 0; i < match.length; i++) {
        if (match[i].DrinkCategoryName == DrinkCategoryName) {
          ...
          res.status(400).json({
            message: "This Drink Category Is Already Exits",
            status: "400",
          });
        }
      }
