...
let findProduct=await ItemRatings.find({_id:productId})
let ZeroPointFive=0;
let OnePointFive=0;
let TwoPointFive=0;
...
let OverAllProuctRate=(5*five + 4.5*FourPointFive +4*four +3.5*ThreePointFive + 3*three +2.5*TwoPointFive + 2*two +1.5*OnePointFive + 1*One+0.5*ZeroPointFive) /NumberOfReviews;

await ItemRatings.findByIdAndUpdate({_id:findProduct[0]._id},{
