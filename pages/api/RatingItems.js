...
let Times=dates.getHours()+"."+dates.getMinutes()+"-"+(((dates.getHours())>=12)?"Pm":"Am");
// merge ReviwerFields
let array=[{OverAllRate:overallRate,userName:userName,userId:id,Message:userMessage,QualityRate:qualityRate,ServiceRate:serviceRate,PriceRate:priceRate,Time:Times,Date:Dates}];
...
// Number of orders controll
NumberOfOrders=Number(findProduct[0].NumberOfOrders);
NumberOfOrders=NumberOfOrders+1;
// Overall Product Rate Manage
OverAllProuctRate=(5*five + 4.5*FourPointFive +4*four +3.5*ThreePointFive + 3*three +2.5*TwoPointFive + 2*two +1.5*OnePointFive + 1*One+0.5*ZeroPointFive) /NumberOfOrders;
...
await ItemRatings.updateOne({ProductId:productId}, {$push: {ItemsReviwers: array},NumberOfOrders:NumberOfOrders,Rating:OverAllProuctRate});
return res.status(201).json({message:"review Successfully Send"});
}
